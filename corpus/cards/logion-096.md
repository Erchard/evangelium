...
Leaven core вже входить до clean reader з маркером. Форма Фоми коротка і близька до синоптичної притчі, але має "царство Отця", large-loaves result і фінальне "хто має вуха", які лишаються в апараті як можливе збіркове або томине розгортання.
...
Для реконструкції ця логія важлива як частково включений матеріал. Clean reader друкує leaven core, а Father frame, large-loaves result і hearing formula лишаються в апараті. Робоче рішення картки: `INCLUDE_WITH_MARKER` for leaven core; full logion `UNCERTAIN`; позначка для чистого читача: YES; попередньо визначений шар: притча про закваску.
...
Наступна дія: publication polish; Father/ears frame remains apparatus material.
...
- Включене раннє ядро: leaven parable core, printed with marker.
- Вторинні / томині ризики: leaven parable core is controlled, but Thomas large-loaves/ear-of-corn style expansion remains apparatus material.
- Грецька політика: Грецький шар можна використовувати тільки як `Greek retroversion, hypothetical`, не як рукописний свідок.

### Наступна дія

Publication polish: keep the Father frame, large-loaves result, and hearing formula in apparatus.
...
- Current decision: `INCLUDE_WITH_MARKER` for leaven core; full logion `UNCERTAIN`.
- Reader text: YES.
- Greek witness status: Greek retroversion, hypothetical.
...
- Next action: Publication polish: Father/ears frame remains apparatus material.
